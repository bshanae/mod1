...
bool				plane::write_height(const point2<int> &iter, const float &z)
{
	...
	ptr = (float *)get_ptr(iter, mod1_engine::model_data::slot_point, convention_dual_first);
	ptr[1] = z;

	if (is_dual(iter))
	{
		ptr = (float *) get_ptr(iter, mod1_engine::model_data::slot_point, convention_dual_second);
		ptr[1] = z;
		return (true);
	}
